...
class DataValidation:
    ...
    def validate_numeber_of_columns(self,dataframe:pd.DataFrame)-> bool:
        try:
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            status = self.validate_numeber_of_columns(dataframe=train_dataframe)
            if not status:
                error_message = 'Train dataframe does not contain all columns.\n'
            status = self.validate_numeber_of_columns(dataframe=test_dataframe)
            if not status:
